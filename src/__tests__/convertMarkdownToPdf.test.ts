...
describe("convertMarkdownToPdf", () => {
  ...
  describe("input validation", () => {
    it("should throw error for empty markdown", async () => {
      await expect(convertMarkdownToPdf("", { onPaymentRequest: mockPaymentHandler })).rejects.toThrow(
        "Invalid markdown input: must be a non-empty string",
      );
    });

    it("should throw error for non-string markdown", async () => {
      await expect(convertMarkdownToPdf(null as any, { onPaymentRequest: mockPaymentHandler })).rejects.toThrow(
        "Invalid markdown input: must be a non-empty string",
      );
    });

    it("should throw error when no payment handler provided", async () => {
      await expect(convertMarkdownToPdf(mockMarkdown, {})).rejects.toThrow("Payment required but no handler provided.");
    });
  });
  ...
  describe("payment handling via 402 response", () => {
    it("should handle payment required via 402 response status", async () => {
      mockedAxiosPost.mockResolvedValueOnce(mock402Response).mockResolvedValueOnce(mockSuccessResponse);
    ...
    it("should handle payment required via axios error with 402 status", async () => {
      ...
      mockedAxiosPost.mockRejectedValueOnce(axios402Error).mockResolvedValueOnce(mockSuccessResponse);
    ...
    it("should handle offer with empty description", async () => {
      ...
      mockedAxiosPost.mockResolvedValueOnce(responseWithoutDescription).mockResolvedValueOnce(mockSuccessResponse);
  ...
  describe("error handling", () => {
    it("should throw error on non-200/402 response status", async () => {
      ...
      await expect(
        convertMarkdownToPdf(mockMarkdown, {
          onPaymentRequest: mockPaymentHandler,
        }),
  ...
  describe("integration with other functions", () => {
    ...
    it("should pass correct parameters to createConversionPayload", async () => {
      ...
      expect(mockedCreateConversionPayload).toHaveBeenCalledWith(mockMarkdown, "Test Title", "Custom Date");
    });
