...
describe("handlePayment", () => {
  ...
  describe("successful payment flow", () => {
    it("should complete payment flow successfully", async () => {
      ...
      expect(mockedWithTimeout).toHaveBeenCalledWith(expect.any(Promise), M2PDF_TIMEOUTS.PAYMENT);
  ...
  describe("invoice request errors", () => {
    it("should throw Markdown2PdfError on non-200 status", async () => {
      ...
      await expect(handlePayment(mockOffer, mockPaymentHandler)).rejects.toThrow("Failed to fetch invoice: 400");
    });
    ...
    it("should throw Markdown2PdfError on network error", async () => {
      ...
      await expect(handlePayment(mockOffer, mockPaymentHandler)).rejects.toThrow(networkError);
    });
    ...
    it("should throw Markdown2PdfError on timeout during invoice request", async () => {
      ...
      await expect(handlePayment(mockOffer, mockPaymentHandler)).rejects.toThrow(timeoutError);
    });
  });
  ...
  describe("payment handler errors", () => {
    it("should throw Markdown2PdfError when onPaymentRequest is not provided", async () => {
      ...
      await expect(handlePayment(mockOffer, null as any)).rejects.toThrow("Payment required but no handler provided.");
    });

    it("should throw Markdown2PdfError when onPaymentRequest is undefined", async () => {
      mockedAxiosPost.mockResolvedValueOnce(mockInvoiceResponse);

      await expect(handlePayment(mockOffer, undefined as any)).rejects.toThrow(
        "Payment required but no handler provided.",
      );
    });
    ...
    it("should propagate error when payment handler throws", async () => {
      ...
      await expect(handlePayment(mockOffer, failingPaymentHandler)).rejects.toThrow(paymentError);
    });
    ...
    it("should throw error on timeout during payment handling", async () => {
      ...
      mockedWithTimeout.mockResolvedValueOnce(mockInvoiceResponse).mockRejectedValueOnce(timeoutError);
